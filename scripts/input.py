...
import rospy
import numpy as np
from challenge_fundamental_robotics.msg import set_point
...
if __name__=='__main__':
	...
	while not rospy.is_shutdown():
		type = rospy.get_param("/signal/type", default = "No signal type found")
		amplitude = rospy.get_param("/signal/amplitude", default = 5.0)
		freq = rospy.get_param("/signal/freq", default = 0.5)
		sq_up = rospy.get_param("/signal/square/up_limit", default = 5.0)
		sq_down = rospy.get_param("/signal/square/down_limit", default = -5.0)
		step_value = rospy.get_param("/signal/step/value", default = 5.0)
		tiempo = rospy.get_param("/signal/time", default = -1.0)
		...
		if (count_time <= tiempo*hz):
			...
			rospy.set_param('/signal/time', -1.0)
			count_time = 0

		msg.input = signal
		msg.time = tiempo
		pwm_pub.publish(msg)
		rospy.loginfo("type: %s, signal: %f", type, signal)
